...
select d.department_name, concat(e.first_name, e.last_name) as Name, max(e.salary) as salary
...
select employee_id, start_date, end_date, (datediff(end_date, start_date))/365 as years
from job_history;

select e.first_name, d.department_name, max(datediff(j.end_date, j.start_date)) as days
from employees as e
...
select concat(e.first_name, '  ' ,e.last_name) as name, c.country_name, max(e.salary)
from employees as e
join departments as d
on e.department_id = d.department_id
join locations as l
...
select employee_id, first_name, last_name, max(salary), manager_id
from employees
group by manager_id;
...
select c.country_id, 
case
when c.country_id = 'US' Then concat(e.first_name, ' US ', e.last_name)  
...
case 
when d.department_name = 'Marketing' Then  min(salary)
else  max(salary)
end as salary
from departments as d
join employees as e 
